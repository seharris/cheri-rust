...
pub fn test_uninhabited_ret_by_ref() {
    ...
    // CHECK-NEXT: call void @llvm.lifetime.start.p{{[0-9]+}}({{(i64 24, )?}}ptr[[ADDRSPACE]] nonnull %_1)
...
pub fn test_uninhabited_ret_by_ref_with_arg(rsi: u32) {
    ...
    // CHECK-NEXT: call void @llvm.lifetime.start.p{{[0-9]+}}({{(i64 24, )?}}ptr[[ADDRSPACE]] nonnull %_2)
